...
export default function LiveSearch(props) {
  ...
  const filteredCountries = (e) => {
    const filteredData = props.Data.filter((item) =>
      item.city.name.toLowerCase().includes(e.target.value.toLowerCase())
    );
  ...
  return (
    <div className="liveSearch liveSearch-layout">
      <section className="search-form flex ml-12">
        <input
          type="text"
          placeholder="Search..."
          className="search-form__txt"
          onChange={(event) => {
